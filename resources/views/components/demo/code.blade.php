...
@if ($copyable)
    <div class="relative border rounded-lg bg-zinc-50 dark:bg-zinc-800 border-zinc-200 dark:border-white/10"
        x-data="{
            isCopied: false,
            copyCode() {
...
@else
    <div class="border rounded-lg bg-zinc-50 dark:bg-zinc-800 border-zinc-200 dark:border-white/10">
        <div {{ $attributes->merge(['class' => 'w-full overflow-auto' . ' ' . $heightClass]) }}>
            {{ $slot }}
        </div>
    </div>
